...
class DynamicIslandService : Service() {
    ...
    override fun onCreate() {
        ...
        var pendingIntentText = sp.getString("intent", "")
        ...
        Log.d(TAG, "onCreate: $pendingIntentText")
        if (pendingIntentText == "" || pendingIntentText == null) {
            config.intent = null
        } else{
        ...
        if (!logMode) {
            // 等待 3 秒后，移除悬浮窗
            Thread {
                Thread.sleep(config.duration+2000)
                // 结束服务
                stopSelf()
            }.start()
